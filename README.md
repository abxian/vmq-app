...
本维护版应用名称为“神仙云收款监控”，使用独立的云端收款监控图标。
...
本分支支持保留完整服务器协议和端口。NAS 部署可使用后台生成的
`pay.sxnn.de:5443/通讯密钥`，客户端会自动通过
`https://pay.sxnn.de:5443` 发送心跳和支付回调；也兼容
`https://pay.sxnn.de:5443/通讯密钥` 以及原有 HTTP 配置。

### 禁止提示弹窗

主界面的“禁止提示弹窗”开启后会统一屏蔽应用的 Toast 提示，包括心跳、
网络错误和收款监听信息；提示仍会尽量写入运行日志。后台心跳、30 秒自动重试、
通知监听和支付回调均继续运行。关闭开关即可恢复正常提示。
